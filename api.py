...
def api_call(): 
    ...
    for track in tracks:
        ...
        if popularity == 0:
            ...
            for key in features_dict[0]: #loop through and add only the attributes we want
                if key != "type" and key != "id" and key != "uri" and key != "track_href" and key != "analysis_url" and key != "time_signature" and key != "mode" and key != "key":
                    track_data.append(features_dict[0][key])
                    # if first == True:
                    #     header.append(key)
            # first = False
            track_data.append(popularity) # popularity will be the y_train
    ...
    header = ['Track_name', 'danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo', 'duration_ms', 'popularity']
